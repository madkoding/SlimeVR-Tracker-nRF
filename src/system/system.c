...
static int sys_retained_init(void)
{
	...
	if (!reset_pin_reset)  // if reset reason is not by pin reset, system automatically
						   // trusts retained state
	{
...
static void button_thread(void)
{
	...
	while (1)
	{
		...
		if (press_time && k_uptime_get() - press_time > 1000
			&& button_read())  // Button is being held
		{
