...
int mmc_update_odr(float time, float *actual_time)
{
	...
	{  // 1000Hz*0.5ms/1000ms = 50% active
		...
		time = 1.0 / 1000;
			 ...
			 > 100)  // Nominal working state, this should use as low power as possible
...
void mmc_mag_read(float m[3])
{
	...
	while ((~status & 0x01)
		   && k_uptime_get() < timeout)  // wait for oneshot to complete or timeout
	{
...
void mmc_mag_process(uint8_t *raw_m, float m[3])
{
	uint32_t rawMag[3];
	rawMag[0]
		= (uint32_t)(raw_m[0] << 10 | raw_m[1] << 2
					 | (raw_m[6] & 0xC0)
						   >> 6);  // Turn the 18 bits into a unsigned 32-bit value
	rawMag[1]
		= (uint32_t)(raw_m[2] << 10 | raw_m[3] << 2
					 | (raw_m[6] & 0x30)
						   >> 4);  // Turn the 18 bits into a unsigned 32-bit value
	rawMag[2]
		= (uint32_t)(raw_m[4] << 10 | raw_m[5] << 2
					 | (raw_m[6] & 0x0C)
						   >> 2);  // Turn the 18 bits into a unsigned 32-bit value
