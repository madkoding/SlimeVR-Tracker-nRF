...
)
{
	...
	int err = 0;
	//	ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, ICM42688_INT_SOURCE0, 0x00); //
	...
	if (clock_rate > 0)
	{
		...
		//		ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, ICM42688_INTF_CONFIG1, 0x91
	...
	//	ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, ICM42688_GYRO_ACCEL_CONFIG0, 0x44);
	//// set gyro and accel bandwidth to ODR/10 	k_msleep(50); // 10ms Accel, 30ms Gyro
	//startup
...
float icm_temp_read(void)
{
	uint8_t rawTemp[2];
	int err
		= ssi_burst_read(SENSOR_INTERFACE_DEV_IMU, ICM42688_TEMP_DATA1, &rawTemp[0], 2);
	if (err)
	{
		LOG_ERR("Communication error");
	}
