...
static bool
	fifo_pattern_gyro_dominant;  // if more samples are gyro than accel in a pattern
...
int lsm6dsm_init(
	...
	if (spi_ret == 0) {
		LOG_INF("Using SPI interface for LSM6DSM sensor");
	} else {
		LOG_INF("SPI interface not available, using I2C interface for LSM6DSM sensor");
	}
	...
	while (count > 0 && len >= PACKET_SIZE) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (pattern % 3 != 0)  // misaligned!
			{
				...
				err |= ssi_burst_read(
					SENSOR_INTERFACE_DEV_IMU,
					LSM6DSM_FIFO_DATA_OUT_L,
					rawCount,
					(pattern % 3) * 2
				);  // read and discard misaligned axes
				count--;
			}
...
{  // TODO: should be off by the time WOM will be setup
	//	ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, LSM6DSM_CTRL1, ODR_OFF); // set
	//accel off 	ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, LSM6DSM_CTRL2, ODR_OFF); //
	//set gyro off
	...
	err |= ssi_reg_write_byte(
		...
		LSM6DSM_CTRL8,
		0x74
	);  // set HPCF_XL to the lowest bandwidth, enable HP_REF_MODE (set HP_REF_MODE,
	...
	err |= ssi_reg_write_byte(
		...
		LSM6DSM_TAP_CFG,
		0x90
	);  // enable interrupts (keep SLOPE_FDS)
	err |= ssi_reg_write_byte(
		SENSOR_INTERFACE_DEV_IMU,
		LSM6DSM_MD1_CFG,
		0x20
