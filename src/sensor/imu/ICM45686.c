...
// ODR lookup table for cleaner and more maintainable code
typedef struct {
	int min_odr;
...
static const odr_config_t accel_odr_table[]
	= {{3201, ACCEL_ODR_6_4kHz, 6400},
	   {1601, ACCEL_ODR_3_2kHz, 3200},
	   {801, ACCEL_ODR_1_6kHz, 1600},
	   {401, ACCEL_ODR_800Hz, 800},
	   {201, ACCEL_ODR_400Hz, 400},
	   {101, ACCEL_ODR_200Hz, 200},
	   {51, ACCEL_ODR_100Hz, 100},
	   {26, ACCEL_ODR_50Hz, 50},
	   {13, ACCEL_ODR_25Hz, 25},
	   {0, ACCEL_ODR_12_5Hz, 12.5}};

static const odr_config_t gyro_odr_table[]
	= {{3201, GYRO_ODR_6_4kHz, 6400},
	   {1601, GYRO_ODR_3_2kHz, 3200},
	   {801, GYRO_ODR_1_6kHz, 1600},
	   {401, GYRO_ODR_800Hz, 800},
	   {201, GYRO_ODR_400Hz, 400},
	   {101, GYRO_ODR_200Hz, 200},
	   {51, GYRO_ODR_100Hz, 100},
	   {26, GYRO_ODR_50Hz, 50},
	   {13, GYRO_ODR_25Hz, 25},
	   {0, GYRO_ODR_12_5Hz, 12.5}};

static uint8_t calculate_odr_setting(
	int target_odr,
	const odr_config_t* table,
	size_t table_size,
	float* actual_time
) {
	for (size_t i = 0; i < table_size; i++) {
		if (target_odr > table[i].min_odr) {
			*actual_time = 1.0f / table[i].actual_freq;
			return table[i].odr_reg_value;
		}
	}
	// Default to lowest frequency
	*actual_time = 1.0f / table[table_size - 1].actual_freq;
	return table[table_size - 1].odr_reg_value;
}
...
int icm45_update_odr(
	...
	const uint8_t ACCEL_UI_FS_SEL = ACCEL_UI_FS_SEL_16G;
	const uint8_t GYRO_UI_FS_SEL = GYRO_UI_FS_SEL_2000DPS;
	uint8_t ACCEL_MODE, GYRO_MODE, ACCEL_ODR, GYRO_ODR;
	...
	if (gyro_time <= 0) {
		...
		GYRO_ODR = calculate_odr_setting(
			target_odr,
			gyro_odr_table,
			sizeof(gyro_odr_table) / sizeof(gyro_odr_table[0]),
			&gyro_time
		);
	...
	if (last_accel_odr == 0xff || last_gyro_odr == 0xff
		...
		|| (last_gyro_odr == 0 ? 0 : 1)
			   != (GYRO_ODR == 0 ? 0 : 1)) {  // TODO: can't tell difference between
											  // gyro off and gyro standby
		err |= ssi_reg_write_byte(
			SENSOR_INTERFACE_DEV_IMU,
			ICM45686_PWR_MGMT0,
	...
	*gyro_actual_time = gyro_time;
	...
	// Calculate FIFO multiplier based on the fastest active sensor
	float fastest_time = 0;
	if (accel_time > 0 && gyro_time > 0) {
		fastest_time = (accel_time < gyro_time) ? accel_time : gyro_time;
	} else if (accel_time > 0) {
		fastest_time = accel_time;
	} else if (gyro_time > 0) {
		fastest_time = gyro_time;
	}

	fifo_multiplier = (fastest_time > 0) ? fifo_multiplier_factor / fastest_time : 0;
...
uint16_t icm45_fifo_read(uint8_t* data, uint16_t len)  // TODO: check if working
{
	...
	while (packets > 0 && len >= PACKET_SIZE) {
		...
		if (packets > limit) {
			LOG_WRN(
				"FIFO read buffer limit reached, %d packets dropped",
				packets - limit
			);
			packets = limit;
...
int icm45_ext_passthrough(
	bool passthrough
)  // TODO: might need IOC_PAD_SCENARIO_AUX_OVRD instead
{
	int err = 0;
	if (passthrough) {
		err |= ssi_reg_write_byte(
			...
			0x18
		);  // AUX1_MODE_OVRD, AUX1 in I2CM Bypass, AUX1_ENABLE_OVRD, AUX1 enabled
	} else {
		err |= ssi_reg_write_byte(
			SENSOR_INTERFACE_DEV_IMU,
			ICM45686_IOC_PAD_SCENARIO_AUX_OVRD,
