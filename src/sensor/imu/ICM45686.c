...
int icm45_init(
	...
	ireg_buf[2]
		= 0xB6 & ~0x92;  // disable internal pull resistors for AP pins (pin 7, 1, 14)
	err |= ssi_burst_write(
		SENSOR_INTERFACE_DEV_IMU,
		ICM45686_IREG_ADDR_15_8,
		ireg_buf,
	...
	k_msleep(50);  // 10ms Accel, 30ms Gyro startup - CRITICAL: reduced delay was
				   // causing random timeouts
	err |= ssi_reg_write_byte(
		SENSOR_INTERFACE_DEV_IMU,
...
uint16_t icm45_fifo_read(uint8_t* data, uint16_t len)  // TODO: check if working
{
	...
	while (packets > 0 && len >= PACKET_SIZE) {
		...
		// Check for FIFO near-full or overflow condition (2048 bytes = 102 packets of
		// 20 bytes)
		if (packets >= 100) {
			LOG_WRN(
				"FIFO near full: %d packets (%d bytes) - possible overflow",
				packets,
				packets * PACKET_SIZE
			);
...
uint8_t icm45_setup_DRDY(uint16_t threshold) {
	...
	buf[1] = threshold >> 8;
	int err
		= ssi_burst_write(SENSOR_INTERFACE_DEV_IMU, ICM45686_FIFO_CONFIG1_0, buf, 2);
	err |= ssi_reg_write_byte(
		SENSOR_INTERFACE_DEV_IMU,
