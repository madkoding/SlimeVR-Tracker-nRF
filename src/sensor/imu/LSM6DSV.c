	...
	while (count > 0 && len >= PACKET_SIZE)
	{
		...
		count = (uint16_t)((rawCount[1] & 3) << 8
						   | rawCount[0]);  // Turn the 16 bits into a unsigned 16-bit
											...
											// but we mask 2nd bit too // TODO: might be
											// 3 bits not 2
...
void lsm_accel_read(float a[3])
{
	uint8_t rawAccel[6];
	int err
		= ssi_burst_read(SENSOR_INTERFACE_DEV_IMU, LSM6DSV_OUTX_L_A, &rawAccel[0], 6);
	if (err)
	{
		LOG_ERR("Communication error");
	}
...
void lsm_gyro_read(float g[3])
{
	uint8_t rawGyro[6];
	int err
		= ssi_burst_read(SENSOR_INTERFACE_DEV_IMU, LSM6DSV_OUTX_L_G, &rawGyro[0], 6);
	if (err)
	{
		LOG_ERR("Communication error");
	}
...
float lsm_temp_read(void)
{
	uint8_t rawTemp[2];
	int err
		= ssi_burst_read(SENSOR_INTERFACE_DEV_IMU, LSM6DSV_OUT_TEMP_L, &rawTemp[0], 2);
	if (err)
	{
		LOG_ERR("Communication error");
	}
...
{  // TODO: should be off by the time WOM will be setup
	//	ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, LSM6DSV_CTRL1, ODR_OFF); // set
	//accel off 	ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, LSM6DSV_CTRL2, ODR_OFF); //
	//set gyro off
	...
	int err = ssi_reg_write_byte(
		...
		LSM6DSV_CTRL8,
		0xE0 | FS_XL_8G
	);  // set accel FS, set HP_LPF2_XL_BW to lowest bandwidth, enable HP_REF_MODE (set
		// HP_LPF2_XL_BW)
	err |= ssi_reg_write_byte(
		SENSOR_INTERFACE_DEV_IMU,
		LSM6DSV_CTRL1,
...
int lsm_ext_setup(void)
{
	...
	int err = ssi_reg_write_byte(
		...
		LSM6DSV_IF_CFG,
		0x58
	);  // SHUB_PU_EN, INT H_LACTIVE active low, PP_OD open-drain
	if (err)
	{
		LOG_ERR("Communication error");
	}
...
int lsm_ext_write(const uint8_t addr, const uint8_t *buf, uint32_t num_bytes)
{
	...
	//	err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, LSM6DSV_SLV0_ADD, (addr <<
	//1) | 0x00); // write 	err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU,
	//LSM6DSV_SLV0_SUBADD, buf[0]); 	err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU,
	//LSM6DSV_SLV0_CONFIG, 0xC0 | 0x00); // SHUB_ODR = 440Hz, reading no bytes
	err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, LSM6DSV_DATAWRITE_SLV0, buf[1]);
	err |= ssi_reg_write_byte(
		SENSOR_INTERFACE_DEV_IMU,
		LSM6DSV_MASTER_CONFIG,
		0x44
	);  // WRITE_ONCE, enable I2C master
...
)
{
	...
	err |= ssi_burst_write(SENSOR_INTERFACE_DEV_IMU, LSM6DSV_SLV0_ADD, slv0, 3);
	//	err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, LSM6DSV_SLV0_ADD, (addr <<
	//1) | 0x01); // read 	err |= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU,
	//LSM6DSV_SLV0_SUBADD, ((const uint8_t *)write_buf)[0]); 	err |=
	...
	err |= ssi_reg_write_byte(
		...
		LSM6DSV_MASTER_CONFIG,
		0x44
	);  // WRITE_ONCE mandatory for read, enable I2C master
	// Wait for transaction
	err |= ssi_reg_write_byte(
		SENSOR_INTERFACE_DEV_IMU,
		LSM6DSV_FUNC_CFG_ACCESS,
