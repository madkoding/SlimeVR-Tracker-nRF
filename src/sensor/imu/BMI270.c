...
// TODO: gyro rotation data is delayed for some reason, accelerometer still responds
// instantly
uint16_t bmi_fifo_read(uint8_t *data, uint16_t len)
{
	...
	while (packets > 0 && len >= PACKET_SIZE)
	{
		...
		err |= ssi_burst_read(
			SENSOR_INTERFACE_DEV_IMU,
			BMI270_FIFO_LENGTH_0,
			&rawCount[0],
			2
		);
		uint16_t count = (uint16_t)((rawCount[1] & 0x3F) << 8
									| rawCount[0]);  // Turn the 16 bits into a unsigned
													 // 16-bit value
...
static int asic_init(void)
{
	uint8_t status;
	int err
		= ssi_reg_read_byte(SENSOR_INTERFACE_DEV_IMU, BMI270_INTERNAL_STATUS, &status);
...
int bmi_crt(uint8_t *data)
{
	...
	*ptr = 0x0100;  // CRT will be executed
	int err
		= ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, BMI270_CMD, 0xB6);  // softreset
	k_msleep(2);
