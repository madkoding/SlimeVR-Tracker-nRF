...
)
{
	...
	int err = ssi_reg_write_byte(
		...
		// multi-byte access (BDU, IF_INC), INT H_LACTIVE active low, PP_OD open-drain
	if (err)
	{
...
{  // TODO: should be off by the time WOM will be setup
	...
	err |= ssi_reg_write_byte(
		...
		LSM6DSO_CTRL8,
		0xF4
	);  // set HPCF_XL to the lowest bandwidth, enable HP_REF_MODE (set HP_REF_MODE_XL,
		// HP_SLOPE_XL_EN, HPCF_XL nonzero)
	err |= ssi_reg_write_byte(
		SENSOR_INTERFACE_DEV_IMU,
		LSM6DSO_TAP_CFG0,
		0x10
...
int lsm6dso_ext_write(const uint8_t addr, const uint8_t *buf, uint32_t num_bytes)
{
	...
	// LSM6DSV_SLV0_SUBADD, buf[0]); 	err |=
	// ssi_reg_write_byte(SENSOR_INTERFACE_DEV_IMU, LSM6DSV_SLV0_CONFIG, 0x00 | 0x00);
	// // SHUB_ODR = 104Hz, reading no bytes
...
)
{
	...
	err |= ssi_reg_write_byte(
		...
		LSM6DSV_MASTER_CONFIG,
		0x4C
	);  // WRITE_ONCE mandatory for read, SHUB_PU_EN, enable I2C master
	// Wait for transaction
	err |= ssi_reg_write_byte(
		SENSOR_INTERFACE_DEV_IMU,
		LSM6DSV_FUNC_CFG_ACCESS,
...
const sensor_ext_ssi_t sensor_ext_lsm6dso
	= {*lsm6dso_ext_write, *lsm6dso_ext_write_read, 8};
