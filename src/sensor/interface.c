...
#define MAX_COMM_ERRORS 20  // Reduced from 100 to detect issues earlier
#define EARLY_WARNING_ERRORS 5  // Warn early to catch intermittent issues
...
void sensor_interface_register_sensor_mag_i2c(
	struct i2c_dt_spec* dev
)  // also used for passthrough
{
	...
	switch (sensor_interface_dev_spec[SENSOR_INTERFACE_DEV_IMU]) {
		case SENSOR_INTERFACE_SPEC_SPI:
			if (ext_ssi != NULL) {
				...
				ext_addr = addr;
				sensor_interface_dev_spec[SENSOR_INTERFACE_DEV_MAG]
...
void sensor_interface_ext_configure(const sensor_ext_ssi_t* ext) { ext_ssi = ext; }

const sensor_ext_ssi_t* sensor_interface_ext_get(void) { return ext_ssi; }
...
int ssi_write(enum sensor_interface_dev dev, const uint8_t* buf, uint32_t num_bytes) {
	switch (sensor_interface_dev_spec[dev]) {
		case SENSOR_INTERFACE_SPEC_SPI:
			...
			printk(
				"ssi_write: %zuB, %.2f MB/s\n",
				num_bytes,
				(double)num_bytes / (double)k_ticks_to_us_near64(end - start)
			);
			return err;
#else
...
int ssi_write_read(
	...
	switch (sensor_interface_dev_spec[dev]) {
		case SENSOR_INTERFACE_SPEC_SPI:
...
#if DEBUG
			LOG_DBG(
				"ssi_write_read: dev=%d, num_write=%zu, num_read=%zu",
				dev,
				num_write,
				num_read
			);
			...
			LOG_HEXDUMP_DBG(
				rx_tmp,
				num_write + sensor_interface_dev_spi_dummy_reads[dev],
				"ssi_write_read: rx_tmp"
			);
			...
			int64_t end = k_uptime_ticks();
			printk(
				"ssi_write_read: %zuB, %.2f MB/s\n",
				(num_write + num_read),
				(double)(num_write + num_read)
					/ (double)k_ticks_to_us_near64(end - start)
			);
			return err;
#else
			{
				...
				if (err != 0) {
					...
					if (consecutive_comm_errors >= EARLY_WARNING_ERRORS
						&& consecutive_comm_errors < MAX_COMM_ERRORS) {
						LOG_WRN(
							"SPI comm errors detected: %d (early warning)",
							consecutive_comm_errors
						);
					} else if (consecutive_comm_errors >= MAX_COMM_ERRORS) {
						LOG_ERR(
							"Critical: %d consecutive SPI comm errors - bus may need "
							"recovery",
							consecutive_comm_errors
						);
					}
				} else {
					if (consecutive_comm_errors > 0) {
						LOG_INF(
							"SPI communication recovered after %d errors",
							consecutive_comm_errors
						);
					}
		...
		case SENSOR_INTERFACE_SPEC_I2C: {
			...
			if (err != 0) {
				...
				if (consecutive_comm_errors >= EARLY_WARNING_ERRORS
					&& consecutive_comm_errors < MAX_COMM_ERRORS) {
					LOG_WRN(
						"I2C comm errors detected: %d (early warning)",
						consecutive_comm_errors
					);
				} else if (consecutive_comm_errors >= MAX_COMM_ERRORS) {
					LOG_ERR(
						"Critical: %d consecutive I2C comm errors - bus may need "
						"recovery",
						consecutive_comm_errors
					);
				}
				...
				if (consecutive_comm_errors > 0) {
					LOG_INF(
						"I2C communication recovered after %d errors",
						consecutive_comm_errors
					);
				}
				consecutive_comm_errors = 0;
		...
		case SENSOR_INTERFACE_SPEC_EXT:
			if (ext_ssi != NULL) {
				...
				int err = ext_ssi->ext_write_read(
					ext_addr,
					write_buf,
					num_write,
					read_buf,
				...
				if (err != 0) {
					...
					if (consecutive_comm_errors >= EARLY_WARNING_ERRORS
						&& consecutive_comm_errors < MAX_COMM_ERRORS) {
						LOG_WRN(
							"EXT comm errors detected: %d (early warning)",
							consecutive_comm_errors
						);
					} else if (consecutive_comm_errors >= MAX_COMM_ERRORS) {
						LOG_ERR(
							"Critical: %d consecutive EXT comm errors - bus may need "
							"recovery",
							consecutive_comm_errors
						);
					}
					...
					if (consecutive_comm_errors > 0) {
						LOG_INF(
							"EXT communication recovered after %d errors",
							consecutive_comm_errors
						);
					}
					consecutive_comm_errors = 0;
...
#if DEBUG
			LOG_DBG(
				"ssi_burst_write: dev=%d, start_addr=0x%02X, num_bytes=%d",
				dev,
				start_addr,
				num_bytes
			);
...
// Recovery function for when communication errors accumulate
int ssi_get_consecutive_errors(void) { return consecutive_comm_errors; }

void ssi_reset_error_counter(void) { consecutive_comm_errors = 0; }
...
#elif CONFIG_SOC_NRF52810
	uint32_t maxcnt
		= 1023;  // easyeda-maxcnt-bits = <10>, I2C timeout (>25ms) on higher interval
#else
	uint32_t maxcnt = sensor_interface_dev_spec[dev] == SENSOR_INTERFACE_SPEC_SPI
						? 16383
						: 1023;  // all other SOC have >=14 bits, I2C timeout (>25ms) on
								 // higher interval
#endif
	interval *= maxcnt / interval;  // maximum interval below maxcnt
	while (num_bytes > 0) {
